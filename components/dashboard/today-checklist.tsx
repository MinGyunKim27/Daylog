...
export function TodayChecklist(props: Props) {
  const done = items.filter((i) => props[i.key]).length
    ...
    <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-4">
      ...
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold">오늘 기록 현황</span>
      ...
      <div className="h-1 bg-[hsl(var(--muted))] rounded-full overflow-hidden mb-3">
        <div className="h-full bg-[hsl(var(--primary))] rounded-full transition-all duration-500"
          style={{ width: `${(done / 5) * 100}%` }} />
      </div>

      {/* 5개 항목 — 가로로 나란히, 높이 최소화 */}
            ...
            <div key={key}
              ...
              <span className="ml-auto shrink-0">
                {checked
                  ...
                  : <Circle size={13} className="text-[hsl(var(--muted-foreground))]" />}
              </span>
            </div>
          )
        })}
